...
int redirection( char temp_command[512], char current_path[512], char* cd_string, char* ls_string, char* pwd_string, char* history_string, char* exit_string,  struct command* tail)
{
...
//        int count_space0 = 0, count_space1 = 0;
        ...
        sperate_command_by_space_pipe_to_string_array(first_sperate_by_redirect, temp_redirect, ">");
        ...
        if (g > l)
        {
          ...
          sperate_command_by_space_pipe_to_string_array(second_sperate_by_redirect, first_sperate_by_redirect[0], "<");
        ...
        if (g < l)
        {
          ...
          sperate_command_by_space_pipe_to_string_array(second_sperate_by_redirect, first_sperate_by_redirect[1], "<");
        ...
        for (i = 0; i < 3; i++)
        {
          memset(com_argument_between_redirect[i], '\0', strlen(com_argument_between_redirect[i]));
          strcpy(com_argument_between_redirect[i] , ptr[i]);
//          printf("com argument between redirect[%d] = %s\n",i, com_argument_between_redirect[i]);
        }
        ...
        char* com_argument0[count_space0 + 2];
        assign_null_to_string_array(com_argument0, count_space0 + 2);
        sperate_command_by_space_pipe_to_string_array(com_argument0, com_argument_between_redirect[0], " ");
        ...
        sperate_command_by_space_pipe_to_string_array(com_argument1, com_argument_between_redirect[1], " ");
        sperate_command_by_space_pipe_to_string_array(com_argument2, com_argument_between_redirect[2], " ");
...
//        printf("%s%s%s\n",com_argument0[0],com_argument1[0],com_argument2[0]);
        int f = 0, fd_out, fd_in, status;
        int pipe_re_fd[2][2];
        int run2 = 1;
...
//    write(1, "\n", 1);
    ...
    { int p_itr, q_itr, m;
      ...
      for (m = 0; m < count_pipe + 1; m++)
        {
          memset(temp_com_argument_between_pipe[m], '\0', strlen(temp_com_argument_between_pipe[m]));
          strcpy(temp_com_argument_between_pipe[m] , com_argument_between_pipe[m]);
        }
...
//          printf("%d %d %d\n", t, pipe_fd[t][0], pipe_fd[t][1]); // why the fd number keep growing
          ...
          run3 = redirection( temp_com_argument_between_pipe[t], current_path, cd_string, ls_string, pwd_string, history_string, exit_string, tail);
          if (!run3)
            exit(0);
          else
          {
    ...
    else
    {
      ...
      {
        redirection( temp_command, current_path, cd_string, ls_string, pwd_string, history_string, exit_string, tail);
      }
